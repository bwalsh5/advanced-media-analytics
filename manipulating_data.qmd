...
```{r, mtcars1, echo = FALSE, include = FALSE}
data(mtcars)
#View(mtcars)
```
...
So as to avoid any complication, let's continue to use the *babynames* dataset for now; then we can move on to analyzing lyrics, political speeches, and great works of literature.
