...
Put another way, a package needs only be installed once on your computer, but has to be loaded into your R environment every time you start up R Studio.
...
Anyone can make and share an R package, for instance if they solve a complicated workflow and want to share that insight with others to save them time and redundancy.
Having our coding environment be package-based means that users only need to load packages relevant to their area of study - keep in mind R is used for everything from Health Sciences to Comparative Literature.
...
Throughout this book, we'll be relying on a collection of inter-connected packages called the Tidyverse, which drastically simplifies performing varying calculations on data.
...
The greatest and most powerful functionality in R, in my opinion, is the 'pipe operator:'

|>

...which allows us to chain commands to each other.
It can be tricky to type; the shortcut is Shift-Command-M (on a PC, it'd be Shift-Control-M). Note that as of R 4.1, `|>` is built into base R — you don't need to load any package to use it.
...
```{r, babynames_ggplot1}
babynames |> filter(name %in% 'Ringo') |> ggplot(aes(year, prop)) + geom_line()
```

To review, we take 'babynames,' and filter so the names only include 'Ringo;' We then plot by setting the aesthetics to use the 'year' and 'prop' columns of our dataset; our plot will be a line chart.
...
We've added a third variable to our plot, with 'name' showing as a color.
We have four discrete names, so R will provide us with four discrete colors.

But the plot looks really weird!
Why?
In summary, because there is a 'Sex' column in the data.
...
#you can also try 'Joseph' and 'Elizabeth'
```
...
Since then, there are just more names, meaning the proportion of the most common names of long ago have nearly all gone down in prop - even if their n value is increasing.

To show what I mean, here's a plot counting every unique name in the data set by year - you can see that the overall amount of unique names is drastically increasing:
...
Because line charts only work with continous variables, like year.
In this case, we are only plotting the data from one year, so year is no longer something we can plot.
...
babynames_1890 |> 
  arrange(desc(prop)) |> 
  head(10) |> 
  ggplot(aes(reorder(name, -prop), prop, color = "red")) + 
  geom_col()
```
...
babynames_1890 |> 
  arrange(desc(prop)) |> 
  head(10) |> 
  ggplot(aes(reorder(name, prop), prop, fill = prop)) + 
  geom_col() + 
  coord_flip()
```
...
babynames_1890 |> 
  arrange(desc(prop)) |> 
  head(10) |> 
  ggplot(aes(reorder(name, prop), prop, fill = n)) + 
  geom_col() + 
  coord_flip()
```
